...
const getNextMovement = (lastMove) => {
    switch (lastMove) {
        case MOVE_NORTH:
            return MOVE_EAST;
        case MOVE_EAST:
...
const updateField = (area, pos, input, output) => {
    // TODO
}

const setWall = (area, pos, lastInput) => {
    const newPos = { ...pos };
    switch (lastInput) {
        case MOVE_NORTH:
            newPos.y -= 1;
...
const drawField = (area, pos) => {
    console.log('\033[2J');
    console.log("+" + "-".repeat(area.length) + "+");
    for (let i = 0; i < area.length; ++i) {
        console.log("|" + area[i].join("") + "|");
    }
    console.log("+" + "-".repeat(area.length) + "+");
}

const findFewestMovements = (programCode) => {
    const program = createProgram([...programCode]);
    let isRunning = true;
    let output = [];
    let newInput = MOVE_NORTH;
    let lastInput;
    const areaSize = 10;
    const pos = { x: 5, y: 5 };
    const area = new Array(areaSize);
    for (let i = 0; i < area.length; ++i) {
        area[i] = new Array(areaSize).fill(" ");
    }
    ...
    do {
        lastInput = newInput;
        let newOutput = program.run(newInput);
        updateField(area, pos, newInput, newOutput);
        ...
        if (newOutput === EXT_HALT) {
            ...
            switch (newOutput) {
                case STATUS_WALL:
                    setWall(area, pos, lastInput);
                    newInput = getNextMovement(lastInput);
                    break;
                case STATUS_MOVE:
                    newInput = lastInput;
                    // TODO: Update pos
...
const result = findFewestMovements(programCode);
